...
class RTOFSNCEPProvider:
    ...
    def _live_frame(self, bbox: BBox) -> tuple[OceanFieldFrame, ProviderStatus]:
        ...
        # TODO: implement real bounded NOMADS/NetCDF subset parsing; never download giant whole-world files in request paths.
        # Current live path is only a remote availability probe/live_stub and returns mock-shaped fields.
    ...
    def _metadata(self, bbox: BBox, source: str, live_ok: bool, cache_status: str, degraded: bool = False, error: str | None = None) -> dict[str, Any]:
        return {
            ...
            "request_url_example": build_rtofs_url(self.settings.rtofs_nomads_base, bbox, self.depth_levels, self.settings.rtofs_max_grid_points),
            "aliases": aliases_used(),
            "normalized_channels": ["sst_c", "current_u", "current_v", "current_speed", "current_direction", "salinity", "depth_m", "bait_score"],
    ...
    def _status_from_frame(self, frame: OceanFieldFrame, live_ok: bool, cache_hit: bool, degraded: bool, source: str, error: str | None = None) -> ProviderStatus:
        return ProviderStatus(
            ...
            generated_time=now_iso(),
            error=error,
            details={"source": source, "provider_id": "rtofs_ncep_ocean", "adapter_status": "live_stub" if live_ok else "mock_fallback", "parser_status": "bounded_NetCDF_parser_TODO", "variable_aliases": aliases_used(), "grid_shape": list(frame.grid_shape), "depth_levels": frame.depth_levels},
        )
